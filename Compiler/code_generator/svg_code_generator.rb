...
class SvgCodeGenerator
  ...
  def generate_chart ast
    @file.write()
    ast.accept(self, nil)
    AnimationInstructions::HTMLFileChart(@filename)
  end
  ...
  def visit_single_line single_line, arg
    single_line.description.accept(self, single_line)
    return nil
  end
  ...
  def visit_timestamp timestamp, arg
    temp_time = timestamp.integer.accept(self, nil).to_i
    unit = timestamp.unit.accept(self, nil)
    case unit
    when 'sec'
  ...
  def visit_description description, arg #arg = single_line
    if (description.identifier.declaration.nil?) #Only selected objects!
      return nil
    end
    id = description.identifier.accept(self, nil)
    arg.timestamp.accept(self, id) #we visit here because each duration is related to an id
    description.expression.accept(self, id)
    return nil
  end
  ...
  def visit_binary_expression binary_expression, arg
    id = arg
    type_binary_expression = binary_expression.binary_action.accept(self, nil)
    arg1 = binary_expression.arg1.accept(self, nil)
    arg2 = binary_expression.arg2.accept(self, nil)
  ...
  def visit_single_declaration single_declaration, arg
    type = single_declaration.type.accept(self, nil)
    id = single_declaration.identifier.accept(self, nil)
    if (!single_declaration.filename.nil?)
      path = single_declaration.filename.accept(self, nil)
    end
    case type
    when 'move'
